...
#pragma builtin
  ...
  inline void iv_const_shift_l(const int *op1, int *r) {
    ...
    if(!B) {
      ...
      // SHIFT_WORDS: compile-time constant for iv_extend<>. Signed M1 avoids unsigned
      // wrap in Nr-M1-1 with gcc 7+ (see iv_extend specializations for -1, 0, ...).
      enum { SHIFT_WORDS = ((B >> 5) > Nr) ? Nr : (B >> 5) };
      iv_extend<SHIFT_WORDS>(r, 0);
      const int M1 = AC_MIN(N + SHIFT_WORDS, Nr);
      if(s31) {
        unsigned lw = 0;
        for(unsigned i=(unsigned)SHIFT_WORDS; i < (unsigned)M1; i++) {
          unsigned hw = op1[i-(unsigned)SHIFT_WORDS];
        ...
        for(unsigned i=(unsigned)SHIFT_WORDS; i < (unsigned)M1 ; i++)
          r[i] = op1[i-(unsigned)SHIFT_WORDS];
        iv_extend<Nr-M1>(r+M1, r[M1-1] < 0 ? -1 : 0);
      }
    }
  }
