...
def _int_from_trace_token(s):
    """Parse integers written by nvdla_trace_parser.py (%X / hex digits, often without 0x prefix)."""
    s = s.strip()
...
def main():
    ...
    # nvdla_trace_parser.py writes: CHECK_CRC <sync_id> PRI_MEM <base> <size> <crc> NA
    #   CHECK_NOTHING <sync_id> NA 0 0 0 NA   (legacy tools used "1" for memory_type)
    with open('%s/trace_parser_cmd_result_checker_command.log' % args.input, 'r') as rcf:
        for line in rcf:
            ...
            if toks[0] == 'CHECK_NOTHING':
                ...
                trace.syncpt_check_crc(
                    syncpts[toks[1]],
                    _int_from_trace_token(toks[3]),
                    _int_from_trace_token(toks[4]),
                    _int_from_trace_token(toks[5]))
            elif toks[0] == 'CHECK_FILE':
                raise NotImplementedError(
                    'CHECK_FILE is not supported by verif/verilator/nvdla.cpp; use CHECK_CRC or CHECK_NOTHING')
            else:
                raise ValueError(toks[0])
