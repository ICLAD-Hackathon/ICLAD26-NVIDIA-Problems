...
set DESIGN_NAME [env_var_or_default DESIGN_NAME NV_nvdla]

# RTL base directory for NVDLA
set rtl_dir [env_var_or_default RTL_DIR [file join $script_dir .. .. outdir nv_small vmod]]
...
# For NVDLA, we also need include directories
if {[env_var_exists_and_non_empty VERILOG_FILES]} {
  # Convert space-separated string to TCL list
  set VERILOG_FILES [split $::env(VERILOG_FILES)]
} else {
  # Default: use filelist to find all Verilog files
  # This will be populated by reading the filelist or glob
  set VERILOG_FILES [list]
}

# NVDLA include directories (for `include directives)
set INCLUDE_DIRS [env_var_or_default INCLUDE_DIRS ""]

# Parse filelist if provided (for large designs like NVDLA)
set VERILOG_FILELIST [env_var_or_default VERILOG_FILELIST ""]
puts "DEBUG: VERILOG_FILELIST = '$VERILOG_FILELIST'"
...
if {$VERILOG_FILELIST ne "" && ![file exists $VERILOG_FILELIST]} {
  ...
  puts "DEBUG: Resolved filelist to: $VERILOG_FILELIST"
}
...
if {$VERILOG_FILELIST ne "" && [file exists $VERILOG_FILELIST]} {
  ...
  # Parse include dirs (-I) and verilog files (-v or plain .v)
  set inc_dirs [list]
  set vfiles [list]
  ...
  foreach line [split $filelist_content "\n"] {
    ...
    # Skip define flags and other options
    ...
    # Verilog file with -v flag: -v <path> or -v<path>
    ...
    # Plain verilog file (ends with .v, .sv, .vlib)
    if {[regexp {\.(v|sv|vlib)$} $line]} {
  ...
  # Update include dirs and verilog files
  if {[llength $inc_dirs] > 0} {
    set INCLUDE_DIRS [concat $INCLUDE_DIRS $inc_dirs]
  }
  if {[llength $vfiles] > 0} {
    set VERILOG_FILES $vfiles
  }
  
  puts "  Parsed [llength $inc_dirs] include dirs, [llength $vfiles] verilog files"
}
...
puts "SDC:     $SDC_FILE"
puts "Include Dirs: [llength [split $INCLUDE_DIRS]] directories"
puts "RTL Files: [llength $VERILOG_FILES] files"
if {[llength $VERILOG_FILES] <= 10} {
  foreach f $VERILOG_FILES {
    puts "  - [file tail $f]"
  }
} else {
  puts "  (first 5 files)"
  for {set i 0} {$i < 5 && $i < [llength $VERILOG_FILES]} {incr i} {
    puts "  - [file tail [lindex $VERILOG_FILES $i]]"
  }
  puts "  ... and [expr {[llength $VERILOG_FILES] - 5}] more"
}
puts "=============================================="
...
set ABC_DRIVER_CELL BUFx4_ASAP7_75t_${VT_TAG}
...
# Build include directory arguments for read_verilog
set include_args ""
if {$INCLUDE_DIRS ne ""} {
  foreach inc_dir [split $INCLUDE_DIRS] {
    if {$inc_dir ne "" && [file exists $inc_dir]} {
      append include_args "-I$inc_dir "
    }
  }
  puts "Include paths configured: [llength [split $INCLUDE_DIRS]] directories"
}

# Define synthesis macros for NVDLA
set defines "-DNO_PLI_OR_EMU -DNO_PLI -DDESIGNWARE_NOEXIST -DSYNTHESIS"
...
foreach file $VERILOG_FILES {
  ...
  if {[catch {eval read_verilog -sv -defer $defines $include_args $file} err]} {
    puts "  ERROR reading $file: $err"
  } else {
    incr read_count
    if {$read_count <= 10 || $read_count == [llength $VERILOG_FILES]} {
