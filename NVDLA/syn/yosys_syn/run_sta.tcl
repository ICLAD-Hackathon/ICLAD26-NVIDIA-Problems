#
# OpenSTA timing analysis script for post-synthesis timing reports
...
#   ASAP7_LIB_DIR=/path/to/libs VT=RVT CORNER=TT sta run_sta.tcl
#
# Output files:
#   reports/sta_timing_report.txt     - Main timing summary with tables
#   reports/sta_timing_paths.txt      - Detailed timing path reports
#   reports/sta_power_report.txt      - Power analysis report
#
...
set DESIGN_NAME [env_or_default DESIGN_NAME NV_nvdla]
...
if {[file exists $sdc_file]} {
  ...
  puts "WARNING: SDC file not found, using default clocks"
  # NVDLA has two clock domains:
  #   - dla_core_clk: Main processing clock (1 GHz = 1000 ps period)
  #   - dla_csb_clk:  Configuration/CSB interface clock (500 MHz = 2000 ps period)
  create_clock -name dla_core_clk -period 1000 [get_ports dla_core_clk]
  create_clock -name dla_csb_clk -period 2000 [get_ports dla_csb_clk]
...
# Output file paths
# ==============================================================================
set timing_report_file [file join $reports_dir sta_timing_report.txt]
set timing_paths_file [file join $reports_dir sta_timing_paths.txt]
set power_report_file [file join $reports_dir sta_power_report.txt]
...
# Run STA command with filtering - removes gated clock and async reset paths
# Filters out timing paths based on Path Group or endpoint patterns
proc sta_cmd_to_fh_filtered { fh cmd } {
  global reports_dir
  set tmpfile [file join $reports_dir .sta_tmp.txt]
  ...
  if {[file exists $tmpfile]} {
    ...
    # Split into individual path reports (separated by blank lines and headers)
    # Filter out paths with "Path Group: gated clock" or "Path Group: asynchronous"
    # or paths ending at SETN/RESETN pins
    ...
    foreach line $lines {
      # Detect start of a new path (Startpoint:)
      if {[regexp {^Startpoint:} $line]} {
        # Output previous path if not skipped
        if {$current_path ne "" && !$skip_path} {
          append filtered_content $current_path
        } elseif {$skip_path} {
          incr filtered_count
        }
        set current_path "$line\n"
        set skip_path 0
        set in_path 1
        continue
      }
      ...
      if {$in_path} {
        append current_path "$line\n"
        
        # Check for patterns to filter out
        if {[regexp {Path Group:\s*(gated clock|asynchronous)} $line]} {
          set skip_path 1
        }
        if {[regexp {/SETN\s|/RESETN\s} $line]} {
          set skip_path 1
        }
        if {[regexp {clock gating-check end-point} $line]} {
          set skip_path 1
        }
        
        # Detect end of path (slack line or empty line after data)
        if {[regexp {slack \(} $line]} {
          # Path complete, will be processed on next Startpoint or EOF
        }
      } else {
        # Not in a path block, output directly
        append filtered_content "$line\n"
      }
    }
    ...
    puts -nonewline $fh $filtered_content
    
    if {$filtered_count > 0} {
      puts $fh ""
      puts $fh "(Filtered $filtered_count gated clock / async reset paths)"
    }
...
# ERC Detailed Report (filtered)
# Filter out async reset pin violations (SETN/RESETN) which are expected
# at pre-layout stage due to high fanout. These will be fixed by reset
# tree buffering during physical design.
if {$slew_violations > 0 || $cap_violations > 0 || $fanout_violations > 0} {
  tee "ERC Violators (excluding async reset pins SETN/RESETN):"
  tee ""
  
  # Capture raw ERC report and filter
  set tmpfile [file join $reports_dir .erc_tmp.txt]
  if {[catch {report_check_types -max_slew -max_capacitance -max_fanout -violators > $tmpfile} err]} {
    tee "Error running ERC check: $err"
  } else {
    if {[file exists $tmpfile]} {
      set f [open $tmpfile r]
      ...
      set shown_count 0
      foreach line $lines {
        # Filter out lines containing SETN, RESETN, or sync_reset_synced_rstn
        if {[regexp {/SETN|/RESETN|sync_reset_synced_rstn} $line]} {
          incr filtered_count
        } else {
          tee $line
          incr shown_count
      ...
      if {$filtered_count > 0} {
        tee ""
        tee "(Filtered $filtered_count async reset pin violations - known pre-layout artifacts)"
      }
...
tee "  Yosys Synthesis:       syn_results/synth_final_report.txt"
tee ""
...
fwrite $rpt_paths "================================================================================"
fwrite $rpt_paths ""
fwrite $rpt_paths "Top Critical Setup Paths (filtered: excludes gated clock & async reset):"
fwrite $rpt_paths "--------------------------------------------------------------------------------"
# Report worst setup paths with filtering for gated clock and async reset paths
sta_cmd_to_fh_filtered $rpt_paths {report_checks -path_delay max -format full_clock_expanded -fields {slew cap input_pins nets fanout} -digits 4 -endpoint_count 20}
fwrite $rpt_paths ""
...
fwrite $rpt_paths "================================================================================"
fwrite $rpt_paths ""
fwrite $rpt_paths "Top Critical Hold Paths (filtered: excludes gated clock & async reset):"
fwrite $rpt_paths "--------------------------------------------------------------------------------"
# Report worst hold paths with filtering
sta_cmd_to_fh_filtered $rpt_paths {report_checks -path_delay min -format full_clock_expanded -fields {slew cap input_pins nets fanout} -digits 4 -endpoint_count 20}
fwrite $rpt_paths ""
...
if {[llength [all_registers]] > 0} {
  fwrite $rpt_paths "Setup (Max Delay) - Register to Register (filtered):"
  fwrite $rpt_paths "--------------------------------------------------------------------------------"
  sta_cmd_to_fh_filtered $rpt_paths {report_checks -path_delay max -from [all_registers] -to [all_registers] -format full_clock_expanded -digits 4 -endpoint_count 10}
  fwrite $rpt_paths ""
  
  fwrite $rpt_paths "Hold (Min Delay) - Register to Register (filtered):"
  fwrite $rpt_paths "--------------------------------------------------------------------------------"
  sta_cmd_to_fh_filtered $rpt_paths {report_checks -path_delay min -from [all_registers] -to [all_registers] -format full_clock_expanded -digits 4 -endpoint_count 10}
  fwrite $rpt_paths ""
} else {
