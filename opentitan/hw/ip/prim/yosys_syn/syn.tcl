#!/usr/bin/env yosys
#
# Yosys synthesis script for OpenTitan Prim Library with ASAP7 PDK
...
#   DESIGN_NAME           - Top module name (default: prim_crc32)
...
#
# Defaults for OpenTitan Prim Library:
...
#   constraint.sdc     - SDC constraints for OpenTitan Prim Library
...
set DESIGN_NAME [env_var_or_default DESIGN_NAME prim_crc32]
...
if {[file exists $design_sdc_file]} {
  set SDC_FILE [env_var_or_default SDC_FILE $design_sdc_file]
} else {
...
# Parse filelist
set VERILOG_FILELIST [env_var_or_default VERILOG_FILELIST [file join $script_dir prim_yosys.f]]
...
if {$SKIP_SV2V} {
  ...
  # - Modules with missing packages (not needed for prim synthesis targets)
  set skip_files [list \
    prim_flop_macros \
    prim_assert \
    prim_ram_1p_adv \
  ...
  foreach file $mod_files {
    ...
    # Skip macro-only files
    if {$module in $skip_files} {
      puts "  Skipped: $module (macro-only file)"
      incr skip_count
      continue
    }
  ...
  if {$skip_count > 0} {
    puts "  Skipped: $skip_count files (macro-only)"
  }
  if {$error_count > 0} {
