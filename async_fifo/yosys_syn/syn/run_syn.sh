#!/bin/bash
#
# Run synthesis and STA for async_fifo
#
# Usage:
#   ./run.sh              # Run synthesis + STA (default)
#   ./run.sh syn          # Run synthesis only
#   ./run.sh sta          # Run STA only (requires prior synthesis)
#   ./run.sh clean        # Clean generated files
#   ./run.sh all          # Run synthesis + STA
#   ./run.sh dry-run      # Show what would be executed without running
#
# Environment variables (optional):
#   VT=RVT|LVT|SLVT       # Voltage threshold (default: RVT)
#   CORNER=TT|FF|SS       # PVT corner (default: TT)
#   ABC_AREA=0|1          # 0=speed, 1=area optimization (default: 0)
...
CORNER=${CORNER:-TT}
ABC_AREA=${ABC_AREA:-0}
DESIGN_NAME=${DESIGN_NAME:-async_fifo}
DRY_RUN=${DRY_RUN:-0}
...
run_sta() {
    ...
    if [[ $DRY_RUN -eq 0 && ! -f "syn_results/${DESIGN_NAME}_syn.v" ]]; then
        ...
        print_info "Run synthesis first: ./run.sh syn"
        exit 1
    fi
...
show_usage() {
    ...
    echo "  ABC_AREA=0|1          0=speed, 1=area optimization (default: 0)"
    echo "  DRY_RUN=1             Enable dry-run mode for any command"
    echo ""
    echo "Examples:"
    echo "  ./run.sh                    # Run synthesis + STA with defaults"
    echo "  ./run.sh syn                # Run synthesis only"
    echo "  ./run.sh sta                # Run STA only"
    echo "  ./run.sh dry-run            # Show what would run without executing"
    echo "  VT=LVT ./run.sh             # Run with LVT cells"
    echo "  ABC_AREA=1 ./run.sh syn     # Run area-optimized synthesis"
    echo "  DRY_RUN=1 ./run.sh syn      # Dry-run synthesis only"
}
